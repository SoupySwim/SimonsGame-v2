...
namespace SimonsGame.Utility
{
	public class PlayerViewport
	{
		...
		public PlayerViewport(Player player, Vector4 viewportBounds, Vector2 levelSize, float scale, GameStateManager manager)
		{
			...
			_countdownOverlay = new InGameOverlay("Ready?\r\n", new Vector4(viewportBounds.W / 6, viewportBounds.Z / 6, 2 * viewportBounds.Z / 3, 2 * viewportBounds.W / 3));
			_pauseOverlay = new InGameOverlay("Paused", new Vector4(viewportBounds.W / 3, viewportBounds.Z / 3, 2 * viewportBounds.Z / 3, 2 * viewportBounds.W / 3));
