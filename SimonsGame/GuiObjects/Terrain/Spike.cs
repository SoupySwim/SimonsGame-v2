			...
			if (!_ignoredIds.ContainsKey(mgo.Id))
			{
				...
				else
				{
					if ((mgo.Center.X - Center.X) * (_isFlipped ? 1 : -1) > 0)
						return;
					horizontalKB = .65f * (_isFlipped ? -1 : 1);
				...
				TickModifier damageFromSpike = new TickModifier(1, ModifyType.Add, this, new Tuple<Element, float>(Element.Normal, 0));
