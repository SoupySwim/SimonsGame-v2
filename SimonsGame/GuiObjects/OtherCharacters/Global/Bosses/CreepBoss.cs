...
namespace SimonsGame.GuiObjects
{
	public class CreepBoss : GenericBoss
	{
		...
		public CreepBoss(Vector2 position, Vector2 hitbox, Group group, Level level)
			: base(position, hitbox, group, level, "Creep Boss")
		{
			Team = Team.Neutral;
			_showHealthBar = true;
			...
			float lowerExp = ((_intensity + 1) * ((100.0f - _overdrivePercent) / 100.0f));
			float upperExp = ((_intensity + _intensityChange + 1) * (_overdrivePercent / 100.0f));
			_abilityManager.Experience = (int)((lowerExp + upperExp) * 22);
		}
					...
					if (_attackBehavior == CreepBossAttackBehavior.WaitBetweenAttacks)
					{
						...
						switch (_attackBehavior)
						{
							...
							case CreepBossAttackBehavior.TeleportAttack:
								...
								TeleportTo(_targetedObject.Center + offset);
								break;
