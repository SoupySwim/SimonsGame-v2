using SimonsGame.GuiObjects.ElementalMagic;
using Microsoft.Xna.Framework;
using SimonsGame;
using SimonsGame.GuiObjects;
using SimonsGame.Modifiers;
using SimonsGame.Utility;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace SimonsGame.Modifiers.Abilities
{
	class ShortRangeMeleeElementalMagicAbility : AbilityModifier
	{
		private PhysicsObject _character;
		private int _tickTotal = 12; // number of ticks the ability will take place. This takes third of a second.
		private int _tickCount = 0; // Where we currently are in the ability.
		private bool _hasStopped = false;
		public bool HasStopped { get { return _hasStopped; } }
		private ShortRangeMeleeMagic _testMagic;

		// This type of modification MAY not do anything to the player.  In the future, it is set up to be possible :D
		public ShortRangeMeleeElementalMagicAbility(PhysicsObject character, Tuple<Element, float> element)
			: base(ModifyType.Multiply, character, element)
		{
			_character = character;
			IsExpiredFunction = IsExpiredFunc;
			Movement = new Vector2(.7f, 1f);
			MaxSpeed = new Vector2(.7f, 1f);
			Damage = -40;
		}

		public bool IsExpiredFunc(GameTime gameTime)
		{
			// When we just start, make the object!
			if (_tickCount == 0)
			{
				//PlayerControls playerControls = GameStateManager.GetControlsForPlayer(_character);
				//Vector2 playerAim = playerControls.GetAim(_character);
				Vector2 playerAim = _character.GetAim();

				// For now, a lot of things are hard coded...
				float posY = _character.Center.Y - 20;
				float posX = playerAim.X > 0 ? _character.Center.X : _character.Center.X - 80;
				_testMagic = new ShortRangeMeleeMagic(new Vector2(posX, posY), _character, new Vector2(80, 40), Group.Passable, _character.Level, playerAim.X > 0, Element, Damage);
				_character.Level.AddGuiObject(_testMagic);
			}
			if (_tickCount == _tickTotal)
			{
				_hasStopped = true;
				_hasReachedEnd = true;
			}
			_tickCount = Math.Min(_tickCount + 1, _tickTotal);

			bool isExpired = _hasStopped;

			if (isExpired)
				_character.Level.RemoveGuiObject(_testMagic);

			return isExpired;
		}
		public override ModifierBase Clone(Guid id)
		{
			ShortRangeMeleeElementalMagicAbility magic = new ShortRangeMeleeElementalMagicAbility(_character, Element);
			magic.Damage = Damage;
			magic._guid = id == Guid.Empty ? Guid.NewGuid() : id;
			return magic;
		}
		public override string GetRange() { return "0"; }
	}
}
