...
namespace SimonsGame.Menu.MenuScreens
{
	...
	public class MapEditorEditMap : MainMenuScreen
	{
		...
		private List<MainGuiObject> _copiedObjects = new List<MainGuiObject>();

		#region Aggregate Select
		private bool _isAddingAggregateObjects = false;
		...
		protected override void DrawExtra(GameTime gameTime, Microsoft.Xna.Framework.Graphics.SpriteBatch spriteBatch)
		{
			...
			spriteBatch.DrawRectangle(_selectPosition, _selectSize, Color.Lerp(Color.White, Color.Transparent, .7f), _scaleAmount);
		...
		private void CheckLeftPanelActions(Vector2 mousePosition)
		{
			...
			{
				_rightPanel.AggregateSelectedObjects.Clear();
				...
				switch (_state)
				{
					...
					case MapEditorState.Select:
						if (Controls.IsClickingLeftMouse())
						{
							...
							_isAddingAggregateObjects = keyboardState.IsKeyDown(Keys.LeftShift) || keyboardState.IsKeyDown(Keys.RightShift);

							Vector2 tempPosition = ((mousePosition - _levelBounds.GetPosition()) / _scaleAmount + _cameraPosition);
							if (_isAddingAggregateObjects && _rightPanel.SelectedObject != null)
								_rightPanel.AggregateSelectedObjects.Add(_rightPanel.SelectedObject);

							_rightPanel.SelectNewItem(_level.GetGuiObjectAtPosition(tempPosition));
							if (_rightPanel.SelectedObject == null || _isAddingAggregateObjects)
							{
								_isAddingAggregateObjects = true;
								_selectPosition = tempPosition;
								_selectSize = Vector2.Zero;
							}
						...
						{
							if (_isAddingAggregateObjects)
							{
								Vector2 tempPosition = ((mousePosition - _levelBounds.GetPosition()) / _scaleAmount + _cameraPosition);
								_selectSize = tempPosition - _selectPosition;
							}
							IsAlteringObject(_rightPanel.SelectedObject, mousePosition, _isResizing, _mouseOffset, _rightPanel.AggregateSelectedObjects.Contains(_rightPanel.SelectedObject));

						}
						else if (Controls.IsReleasingLeftMouse())
						{
							if (_isAddingAggregateObjects)
							{
								KeyboardState keyboardState = Keyboard.GetState();
								bool addToCollection = keyboardState.IsKeyDown(Keys.LeftShift) || keyboardState.IsKeyDown(Keys.RightShift);
								Vector2 tempPosition = ((mousePosition - _levelBounds.GetPosition()) / _scaleAmount + _cameraPosition);
								_selectSize = tempPosition - _selectPosition;
								List<MainGuiObject> containedObjects = _level.GetGuiObjectsInRange(new Vector4(_selectPosition, _selectSize.Y, _selectSize.X));
								if (addToCollection)
									containedObjects.ForEach(mgo => _rightPanel.AggregateSelectedObjects.Add(mgo));
								else
									_rightPanel.AggregateSelectedObjects = new HashSet<MainGuiObject>(containedObjects);


								_selectPosition = Vector2.Zero;
								_selectSize = Vector2.Zero;
							}
							else if (_rightPanel.AggregateSelectedObjects.Any())
							{
								foreach (MainGuiObject mgo in _rightPanel.AggregateSelectedObjects)
									FinishAlteringObject(mgo);
							}
							else
								FinishAlteringObject(_rightPanel.SelectedObject);
						} // if it's releasing, then we finish altering the object.
						break;
				}
			}
		}
		...
		{
			if (_rightPanel.AggregateSelectedObjects.Any())
			{
				_copiedObjects.Clear();
				var aggObjects = _rightPanel.AggregateSelectedObjects.ToList();
				foreach (var original in aggObjects)
				{
					MainGuiObject mgo = (MainGuiObject)original.Clone();
					mgo.Position.X += Level.PlatformDifference;
					mgo.Position.Y += Level.PlatformDifference;
					_copiedObjects.Add(mgo);
				}
			}
			else if (_rightPanel.SelectedObject != null)
			{
				_copiedObjects.Clear();
				MainGuiObject mgo = (MainGuiObject)_rightPanel.SelectedObject.Clone();
				mgo.Position.X += Level.PlatformDifference;
				mgo.Position.Y += Level.PlatformDifference;
				_copiedObjects.Add(mgo);
			}
		...
		{
			if (_copiedObjects.Count() == 1)
			{
				MainGuiObject mgo = (MainGuiObject)_copiedObjects.First().Clone();
			...
			{
				_rightPanel.AggregateSelectedObjects.Clear();
				foreach (var mgoOriginal in _copiedObjects)
				{
					MainGuiObject mgo = (MainGuiObject)mgoOriginal.Clone();
					Level.AddGuiObject(mgo);
					_rightPanel.AggregateSelectedObjects.Add(mgo);
				}
			}
		}
		...
		private bool FinishAlteringObject(MainGuiObject mgo)
		{
			if (mgo == null)
				return false;
			bool didCreate = true;
			if (mgo.Size.X == 0 || mgo.Size.Y == 0) // Bug for connectors?
			{
		...
		private void IsAlteringObject(MainGuiObject mgo, Vector2 mousePosition, bool isAlteringSize, bool doAggregate = false)
		{
			IsAlteringObject(mgo, mousePosition, isAlteringSize, Vector2.Zero, doAggregate);
		}
		private void IsAlteringObject(MainGuiObject mgo, Vector2 mousePosition, bool isAlteringSize, Vector2 mouseOffset, bool doAggregate = false)
		{
			if (mgo == null)
				return;
			...
			if (isAlteringSize)
			{
				moveAmount = tempPosition - mgo.Position - mgo.Size;
				mgo.Size = tempPosition - mgo.Position;
			}
			else
			{
				moveAmount = (tempPosition + (mgo.Center - NormalizePosition(mgo.Center))) - mgo.Center;
				mgo.Center = mgo.Center + moveAmount;
			}
			if (doAggregate)
			{
				foreach (MainGuiObject aggMgo in _rightPanel.AggregateSelectedObjects)
				{
					if (!aggMgo.Equals(mgo))
					{
						if (isAlteringSize)
							aggMgo.Size = aggMgo.Size + moveAmount;
						else
							aggMgo.Center = aggMgo.Center + moveAmount;
					}
				}
			}
		}
