...
namespace SimonsGame.Modifiers
{
	...
	{
		// Must Set Id.
		public PlayerAbilityInfo(Guid id)
		{
			_id = id;
		}
	...
	public class MultiPlayerAbilityInfo : PlayerAbilityInfo
	{
		...
		public MultiPlayerAbilityInfo(Guid id, int count)
			: base(id)
		{
			...
			foreach (KeyValuePair<KnownAbility, List<PlayerAbilityInfo>> pair in _knownAbilities)
			{
				foreach (PlayerAbilityInfo playerAbilityInfo in pair.Value)
				{
					...
					if (CanUseAbility(playerAbilityInfo.Id, playerAbilityInfo))
					{
						...
						_currentAbilities.Add(playerAbilityInfo.GetNextAbilityId(), currentModifier);

						if (ShouldStartCooldownImmediately(playerAbilityInfo))
							_coolDownCounter.Add(playerAbilityInfo.Id, new TimeSpan(1));
